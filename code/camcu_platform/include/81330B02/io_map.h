...
typedef struct io_mlx16_s {
    ...
    union {
        ...
        struct {
            ...
            uint16_t reserved_470 : 10;
        };
    };
    uint16_t reserved_480;
    ...
    union {
        uint16_t ITC_PRIO4 : 2;
        struct {
            uint16_t TX_TIMEOUT_PRIO : 2;
            uint16_t reserved_706 : 14;
        };
    };
    uint16_t reserved_720;
...
typedef struct io_port_comm_cfg_s {
    uint16_t SPI_MOSI_IN_SEL : 2;
    uint16_t reserved_2 : 2;
    uint16_t SPI_MISO_IN_SEL : 2;
    uint16_t reserved_6 : 2;
    uint16_t SPI_SCK_IN_SEL : 2;
    uint16_t reserved_10 : 2;
    uint16_t SPI_SS_IN_SEL : 2;
} io_port_comm_cfg_t;
...
extern volatile io_port_timer_cfg1_t PORT_TIMER_CFG1 __attribute__((nodp, addr(0x23c)));
...
extern volatile io_port_io_trig_edge_cfg_t PORT_IO_TRIG_EDGE_CFG __attribute__((nodp, addr(0x23e)));

typedef struct io_port_tx_timeout_s {
    const uint16_t TX_TIMEOUT : 1;
} io_port_tx_timeout_t;

extern volatile io_port_tx_timeout_t PORT_TX_TIMEOUT __attribute__((nodp, addr(0x240)));
