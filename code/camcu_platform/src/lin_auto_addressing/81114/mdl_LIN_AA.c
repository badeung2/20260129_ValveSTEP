 ...
 *
 * This file is part of the Mlx81114 module library.
 *
 * File mdl_LIN_AA.c
 *
...
#define LIN_AA_STOP()                                                                                \
    do {                                                                                             \
        IO_SET(PORT_LINAA1, LINAA_EN, VAL_LINAAEN_DISABLE,LINAA_CDOUTEN, VAL_LINAA_CDOUTEN_DISABLE); \
        IO_SET(PORT_LINAA2, LCD_DIS_LINAA, VAL_LCD_CURR_ENABLE, LCD_ON_LINAA, VAL_LCD_CURR_Off);     \
    } while (0);
...
#define LIN_AA_SET_CURRENT_OUTPUT()                                                            \
    do  {                                                                                      \
        IO_SET(PORT_LINAA1, LINAA_CDOUTEN, VAL_LINAA_CDOUTEN_ENABLE);                          \
        IO_SET(PORT_LINAA2, LCD_DIS_LINAA,VAL_LCD_CURR_ENABLE, LCD_ON_LINAA, VAL_LCD_CURR_On); \
        MLX81113_USEC_DELAY(1); /*1us*/                                                        \
    } while (0);
...
void l_AutoAddressingStep(uint8_t StepNumber){
    ...
    MLX81113_USEC_DELAY(LINAA_STARTDELAY);
        ...
        switch(StepNumber) {
            ...
            case LIN_AA_BSM_SNPD_STEP1:
                ...
                if((EE_GET(LINAA_GAINDMCAL) == 0) || (EE_GET(LINAA_GAIN) == 0) || (EE_GET(LINAA_LCD_SEL_205) == 0)) {
                    IO_SET(PORT_LINAA2, LCD_SEL_LINAA, VAL_DEFAULT_LCD_SEL_045);
                }
                else {
                    IO_SET(TRIM_MISC, TRIM_LCD_LINAA, EE_GET(LINAA_TRIM_LCD_045));
                    IO_SET(PORT_LINAA2, LCD_SEL_LINAA, EE_GET(LINAA_LCD_SEL_045));
                }
#endif
                break;
...
#else
                if((EE_GET(LINAA_GAINDMCAL) == 0) || (EE_GET(LINAA_GAIN) == 0) || (EE_GET(LINAA_LCD_SEL_205) == 0)) {
                    IO_SET(PORT_LINAA2, LCD_SEL_LINAA, VAL_DEFAULT_LCD_SEL_240);
                }
                else {
                    IO_SET(TRIM_MISC, TRIM_LCD_LINAA, EE_GET(LINAA_TRIM_LCD_240));
                    IO_SET(PORT_LINAA2, LCD_SEL_LINAA, (0x04u | EE_GET(LINAA_LCD_SEL_240)));
                }
#endif
                break;
...
void mdl_LIN_AA_loaddefault(void){
        /* load default values */
        IO_SET(TRIM_MISC, TRIM_LCD_LINAA, VAL_DEFAULT_LCD_TRIM);
#if LINAACSPRESEL == 0
        IO_SET(PORT_LINAA2, LCD_SEL_LINAA, VAL_DEFAULT_LCD_SEL_205);
#else
        IO_SET(PORT_LINAA2, LCD_SEL_LINAA, VAL_DEFAULT_LCD_SEL_045);
#endif
    IO_SET(PORT_LINAA1,
...
#if LINAACSPRESEL == 0
        IO_SET(TRIM_MISC, TRIM_LCD_LINAA, EE_GET(LINAA_TRIM_LCD_205));
        IO_SET(PORT_LINAA2, LCD_SEL_LINAA, EE_GET(LINAA_LCD_SEL_205));
#else
        IO_SET(TRIM_MISC, TRIM_LCD_LINAA, EE_GET(LINAA_TRIM_LCD_045));
        IO_SET(PORT_LINAA2, LCD_SEL_LINAA, EE_GET(LINAA_LCD_SEL_045));
#endif
