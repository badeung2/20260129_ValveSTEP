...
void fw_start(void)
{
    ...
    IO_SET(MLX16, ITC_PRIO6, 0xFFFFu);
    IO_SET(MLX16, ITC_PRIO7, 0xFFFFu);

    /* all phases in tri-state explicitly */
    MotorDriverUVWRSTSelectSource(DRV_CTRL_TRISTATE, DRV_CTRL_TRISTATE, DRV_CTRL_TRISTATE,
                                  DRV_CTRL_TRISTATE, DRV_CTRL_TRISTATE, DRV_CTRL_TRISTATE);

#if defined(HAS_PATCH_ROM)
    /* load flash enabled ROM patches */
    patch_rom_Init();
#endif
